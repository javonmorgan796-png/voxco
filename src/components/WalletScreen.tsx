...
import { X, Wallet, ArrowUpRight, ArrowDownLeft, TrendingUp, Gift, Zap, DollarSign, Clock, CheckCircle2, XCircle, Loader2 } from "lucide-react";
...
const WalletScreen = ({ onClose }: WalletScreenProps) => {
  ...
  const fetchRequests = async () => {
    try {
      ...
      if (dRes.error || wRes.error) {
        throw dRes.error || wRes.error;
      }

      const merged: MyRequest[] = [
        ...((dRes.data ?? []) as Omit<MyRequest, "kind">[]).map((r) => ({ ...r, kind: "deposit" as const })),
        ...((wRes.data ?? []) as Omit<MyRequest, "kind">[]).map((r) => ({ ...r, kind: "withdrawal" as const })),
      ]
        .filter((r) => r && r.id && r.amount_usd !== undefined && r.status)
        .sort((a, b) => new Date(b.created_at).getTime() - new Date(a.created_at).getTime());
      
      setMyRequests(merged);
    } catch (err) {
      console.error("Failed to fetch requests:", err);
      setRequestsError(err instanceof Error ? err.message : "Failed to fetch requests");
      setMyRequests([]);
    } finally {
        ...
        <div className="flex-1 overflow-y-auto p-4 pb-24 space-y-6">
          ...
          >
            <div className="relative z-10">
              ...
              <h2 className="text-4xl font-bold text-white drop-shadow">${(balance ?? 0).toFixed(2)}</h2>
                      ...
                      <div className="flex items-center gap-3">
                        ...
                        <div>
                          <p className="text-sm font-medium text-foreground">
                            ${(r.amount_usd ?? 0).toFixed(2)} {r.crypto} {r.kind}
                          </p>
                ...
                transactions.slice(0, 50).map((tx) => (
                  <div key={tx.id} className="glass-card p-3 flex items-center justify-between">
                    ...
                    <span className={`text-sm font-bold ${isCredit(tx.type) ? "text-green-500" : "text-red-500"}`}>
                      {isCredit(tx.type) ? "+" : "-"}${(tx.amount ?? 0).toFixed(2)}
                    </span>
                  </div>
                ))
              )}
